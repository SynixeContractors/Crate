...
pub async fn handle(msg: Message, client: ArcCacheAndHttp) {
    ...
    match ev {
        executions::Request::UpdateActivityRoles {} => {
            ...
            let active = active
                .into_iter()
                .map(|x| UserId(x.parse::<u64>().expect("Failed to parse UserId")))
                .collect::<Vec<_>>();
            let mut members = GUILD.members_iter(&client.http).boxed();
            while let Some(Ok(mut member)) = members.next().await {
                if active.contains(&member.user.id) && !member.roles.contains(&ACTIVE) {
                    if let Err(e) = member.add_role(&client.http, ACTIVE).await {
                        error!("Failed to add role to {}: {}", member.user.id, e);
                    } else {
                        let _ = LOG
                            .send_message(&client.http, |m| {
                                m.content(format!("<@{}> is now active", member.user.id))
                            })
                            .await;
                    }
                    continue;
                ...
                if !active.contains(&member.user.id) && member.roles.contains(&ACTIVE) {
                    if let Err(e) = member.remove_role(&client.http, ACTIVE).await {
                        error!("Failed to remove role from {}: {}", member.user.id, e);
                    } else {
                        let _ = LOG
                            .send_message(&client.http, |m| {
                                m.content(format!("<@{}> is no longer active", member.user.id))
                            })
                            .await;
                    }
                }
