...
macro_rules! request {
    ($nats:expr, $token:expr, $body:expr) => {{
        use $crate::Evokable;
        let body = $body;
        let path = body.self_path();
        let mut trace_body = $crate::Wrapper::new($token, body);
        ...
        $nats.request_timeout(
            path,
            serde_json::to_vec(&trace_body).unwrap(),
...
macro_rules! publish {
    ($nats:expr, $body:expr) => {{
        ...
        $nats.publish(path, serde_json::to_vec(&trace_body).unwrap())
    }};
}
...
macro_rules! respond {
    ($msg:expr, $resp:expr) => {{
        ...
        $msg.respond(serde_json::to_vec(&$resp).unwrap())
    }};
}
...
macro_rules! parse_data {
    ($msg:expr, $t:ty) => {{
        let wrapper = serde_json::from_slice::<$crate::Wrapper<$t>>(&$msg.data).unwrap();
